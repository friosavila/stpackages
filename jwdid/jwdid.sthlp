...
{pstd}In the default option, only the interactions of cohort and time AFTER treatment occurred are included in the specification. This effectively uses not-yet treated as controls. When using the option {cmd:never}, all posible interactions of cohort 
and time are used, thus considering only the never treated as controls, following Callaway and Sant'Anna (2021) and {cmd: csdid}. 
...
Simple Approaches to Nonlinear Difference-in-Differences with Panel Data. Working paper.{p_end}
...
{p 7 14 2}
Help:  {help drdid}, {help csdid}, {help jwdid_postestimation}, {help xtdidregress} {p_end}

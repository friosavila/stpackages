...
{synopt:{opt i:var(varname)}}Declares the Panel ID variable. If not declared, the data is assumed to be Repeated cross-section. {p_end}
...
{synopt:{opt trtvar(varname)}}If Gvar is not available, you can provide the post-treatment variable. By default, the panel data identifier is used to create the equivalent to Gvar. This option also allows you to add a variable that identifies intensity (0-1) {p_end}
{synopt:{opt trgvar(varname)}}If data is repeated cross-section (RC), {cmd:trtvar()} can be combined with {cmd: trtgvar()} to identify the cohort variable. {cmd:trtgvar} should identify pseudo panels in the data, where each pseudo panel contains observations that were (or could have) been treated at the same point in time{p_end}

{syntab :Basic DID Specification options}
{synopt:{opt never}}By default, {cmd: jwdid} uses all never and not-yet treated observations as controls. The option {cmd: never} requests to use only never-treated as the control group. With this option, for each group/cohort, the period g-1 is excluded from the specification. {p_end}
{synopt:{opt group}}Request using Group fixed effects instead of Panel ID fixed effects. In linear models, with balanced panel, estimates are numerically identical. However, it may reduce the computational burden with nonlinear models. This is the default whenever {cmd: method()} is called for except for {cmd: ppmlhdfe}.{p_end}
...
{synopt:{cre}} This option is used to request a correlated random effects (Mundlak) correction as an alternative to inclusion of fixed effects model. Particularly useful when using non-linear models. Supersedes -corr- {p_end}
...
{synopt:{opt method(method_name)}}Request other methods for the estimation of ATTs. For example {cmd:poisson} or {cmd:logit}. Default is linear regression model, which is estimated using {cmd: reghdfe}.{p_end}
...
{synopt:{opt hettype(hetspec)}}This option allows you to request different types of treatment effect heterogeneity. The default is to use full time-cohort heterogeneity. Other options include {cmd:time}, {cmd:cohort}, {cmd: event} and {cmd:twfe}. The last one reduces to the traditional TWFE estimator {p_end}
...
{synopt:{opt xattvar(varlist)}}Variables declared are only interacted with the group/cohort x time interactions. Useful for treatment heterogeneity analysis.{p_end}

{synopt:{opt fevar(varlist)}}This option is used to add additional fixed effects to the model, different from cohort, panel or time. Only valid when using default method (reghdfe) or if using ppmlhdfe.{p_end}

{synopt:{opt anti:cipation(#1)}}This is used to declare a period different from g-1 as the baseline. #1 can only take integer positive values, default is 1{p_end}
...
{pstd}In addition to imposing constraints on the treatment effect heterogeneity, it is also possible to add covariates that allow for covariate heterogeneity, as well as restricting the heterogeneity only across time, across cohorts or no heterogeneity at all. Any variable that is included in the {cmd:varlist} is interacted with the corresponding level of treatment heterogeneity. The default option is to add sub-group demeaned variables to the model, but it is possible to request using the original variables using the option {cmd: xasis}. Both approaches produce the same aggregate average treatment effects, but the interpretation of the model coefficients changes. 

{pstd}One can also request adding variables that are interacted with the time variable only {cmd: xtvar()}, cohort variable only {cmd: xgvar()}, or not interacted at all {cmd: exovar()}. Similarly, additional fixed effects can be added to the model using the option {cmd: fevar()}.

{pstd}One should also be aware that {cmd: jwdid} will create as many interactions as cohorts and periods there are in the data, times all possible controls, unless heterogeneity is restricted and covariate heterogeneity is restricted using the above options.

{pstd}This could provide an extremely large number of estimated coefficients, which could make some models difficult to estimate.

{pstd}Other estimation methods can be used by declaring the option {cmd: method()}. The default method is {cmd: reghdfe}, which is a linear regression model with fixed effects. Other methods include {cmd: poisson}, {cmd: logit}, or {cmd: ppmlhdfe}, which is the leading approach for the estimation of gravity-trade models. 

{pstd}Except when {cmd: ppmlhdfe} is used, whenever {cmd: method()} is declared, the program will assume group/cohort fixed effects only, instead of individual fixed effects. This is to avoid the incidental parameter problem.
...
{phang2}2. It is flexible for other methods. Namely, you should be easily able to apply it for binary or count models, or even for the estimation of gravity-trade models, as we show in Nagengast et al. (2024).

{pstd}This program was coded independently from Jeff Wooldridge but is based on some of his early simulation codes he shared with the community. Some further advances were made following the official implementation of {help xthdidregress} and {help hdidregress}. Most advances were developed for the joint work with Arne Nagengast and Yoto Yotov, for the estimation of DID-type models in gravity trade settings.
...
{pstd}This command started as an isolated project before becoming a dad! 2 years later and I'm almost ready to get a much better version of it. {p_end}

...
{synopt:{cre}} This option is used to request a correlated random effects (Mundlak) correction as an alternative inclusion of fixed effects model. Particularly useful when using non-linear models. Superseeds -corr- {p_end}
...
{synopt:{opt xattvar(varlist)}}Variables declared are only interacted with the group/cohort x time Interactions. Usefull for treatment heterogeneity analysis.{p_end}
...
{synopt:{opt anti:cipation(#1)}}This is used to declare period different from g-1 as the baseline. #1 can only take integer positive values, default is 1{p_end}

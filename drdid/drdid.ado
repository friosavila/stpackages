* Ver 1.91 Pedro Sant'Anna fixes some typos on IFs, trimming, and correct usage of weights.
* Ver 1.9 Scott Cunningham edits added a few lines of trimming propensity scores above 0.995 for the control groups 
* Ver 1.8 Trimming
...
program define drdid, eclass byable(onecall)
		...
		/**Version**/
		if   "`version'"!="" {
			display "version: 1.91"
			addr scalar version = 1.91
			exit
		}
...
program define drdid_imp, eclass  
	...
	if "`ivar'"!="" {
		qui {
			...
			** determine weights
			** First, winsorize the propensity score
			tempvar pscorefit
			gen double `pscorefit' = logistic(`psxb')
			replace `pscorefit' = (`pscorefit' <= 1-1e-6) * `pscorefit' + (`pscorefit' > 1-1e-6) * (1-1e-6)

			** Then use the winsorized pscore in the weight calculation
			gen double `w0' = ((1-`trt')*`pscorefit'/(1-`pscorefit'))*`weight' 
			...
			** estimating dy_hat for a counterfactual
			...
			gen byte `touse2'=`touse'*(`tmt'==0)
			replace `touse'=0 if `__dy__'==.
			mata:drdid_imp_panel("`__dy__'","`xvar' ","`xb'","`iptb'","`iptV'","`psxb'","`trt'","`tmt'","`touse2'","`att'","`weight'","`trimflag'")
			********************************************************************************				
			********************************************************************************
	...
	**# for Crossection estimator				
		...
		qui {
			...
			** Then use the winsorized pscore in the weight calculation
			...
			matrix `regb01' =e(b)
			matrix `regV01' =e(V)
			`isily' reg `y' `xvar'  [w=`weight']		   if `trt'==1 & `tmt'==0,
			predict double `y10'
			matrix `regb10' =e(b)
			matrix `regV10' =e(V)
			`isily' reg `y' `xvar'  [w=`weight']		   if `trt'==1 & `tmt'==1,
			predict double `y11'
			matrix `regb11' =e(b)
			matrix `regV11' =e(V)
			tempname b V ciband ncl
...
mata 
	////////////////////////////////////////////////////////////////////////////////////////////////
// Improved DR estimator (WLS for outcome regression and logit-based IPT for pscore)
	void drdid_imp_panel(string scalar dy_, xvar_, xb_ , psb_,psV_,psxb_,trt_,tmt_,touse,rif,ww,trimflag_) {
		...
		real matrix psc
		psc=logistic(psxb)
		psc = (psc :<= 1-1e-6) :* psc  :+ (psc :> 1-1e-6) :* (1-1e-6)
		trt =st_data(.,trt_ ,touse)
		trimflag = st_data(.,trimflag_,touse)  // Load trimflag
		real matrix w
		w=st_data(.,ww,touse)
		real matrix w_1 , w_0, att, att_inf_func, att_summand
		att_summand =  trimflag :*(1 :- (1 :- trt):/(1 :- psc)) :* (dy :- xb)
		att = mean(w :* att_summand) :/ mean(trt :* w)
		att_inf_func = att :+ (w :* trimflag :* (att_summand :- trt :*att ) :/ mean(trt :* w))
		st_store(.,rif,touse, att_inf_func)	
	}		
		
 
	// Standardized/Normalized IPW
...
/////////////////////////////////////////////////////////////////////////////////////////////////	
// IPW without Normalized Weights (HT-type estimator as in Abadie DiD Restud paper)
void ipw_abadie_panel(string scalar dy_, xvar_, xb_ , psb_,psV_,psxb_,trt_,tmt_,touse,rif,ww,trimflag_) {
    ...
    w_1= w :* trt 
    w_0= trimflag :* w :* (1 :- trt)  :* (psc:/(1 :- psc)) // Apply trimflag here!
    att_treat = w_1 :* dy
    att_cont  = w_0 :* dy
    eta_treat = mean(att_treat):/mean(w_1)
    eta_cont  = mean(att_cont):/mean(w_1)
    ipw_att   = mean((w_1 :- w_0) :* dy):/mean(w_1)
// 	eta_treat :- eta_cont
    lin_ps = (w:* (trt :- psc) :* xvar)*(psv * nn)
    att_lin1  = att_treat :- att_cont
    mom_logit = mean(att_cont  :* xvar)
    att_lin2  = lin_ps * mom_logit'
    att_inf_func = (ipw_att):+(att_lin1 :- att_lin2 :- w_1 :* ipw_att)/mean(w_1)
    st_store(.,rif,touse, att_inf_func)	
...
//     w_0 = w_0 /mean(w_0 ) 
    ...
    dy_xb=dy:-xb
	dr_att_treat= w_1 :*dy_xb
	dr_att_cont= w_0 :*dy_xb
	eta_treat = mean(dr_att_treat)/mean(w_1)
	eta_cont = mean(dr_att_cont)/mean(w_0)
...
//     att = mean((w_1:-w_0):*(dy_xb))
    ...
    lin_wols =  wols_eX * XpX_inv  // nxk 
    // IF for logit
    lin_ps 	   = (w :* (trt:-psc) :* xvar) * (psv * nn) // nxk
    // Start the Influence functions: first, the treated guys
	inf_treat_1 = dr_att_treat :- (w_1 :* eta_treat) // nx1
	inf_treat_2 = lin_wols * (mean(w_1 :* xvar))' // nx1
	inf_treat = (inf_treat_1 :- inf_treat_2):/ mean(w_1) // nx1
	// Now the comparison groups
	inf_cont_1 = dr_att_cont :- (w_0 :* eta_cont) // nx1
	inf_cont_2 = lin_ps * (mean(w_0 :* (dy_xb :- eta_cont) :* xvar))' // nx1
	inf_cont_3 = lin_wols * (mean(w_0 :* xvar))' // nx1
	inf_cont = (inf_cont_1 :+ inf_cont_2 :- inf_cont_3):/mean(w_0) // nx1
	// Combine the components and add the att (code relies on recentered IF)
	att_inf_func = att:+inf_treat :- inf_cont
    st_store(.,rif,touse, att_inf_func)	
    // Variance should be divided by n not n-1
}
...
void ipw_abadie_rc(string scalar y_, xvar_ , tmt_, trt_, psv_, pxb_, wgt_ ,  touse, rif, trimflag_){
	...
	real matrix inf_logit, att_inf_func, att_inf_func_prelim
...
//    	
    ...
    eta_trt_post   		= w1  :* (y11 :- y01):/mean(w1)
    eta_trtt1_post 		= w11 :* (y11 :- y01):/mean(w11)
    eta_trt_pre   		= w1  :* (y10 :- y00):/mean(w1)
    eta_trtt0_pre 		= w10 :* (y10 :- y00):/mean(w10)
    ...
    att_trtt1_post 		= mean(eta_trtt1_post)
    att_trt_pre   		= mean(eta_trt_pre)
    att_trtt0_pre 		= mean(eta_trtt0_pre)
	
	real matrix trtr_att
    trtr_att      		= (att_treat_post :- att_treat_pre) :- (att_cont_post :- att_cont_pre) :+ (att_trt_post :- att_trtt1_post) :- (att_trt_pre :- att_trtt0_pre)

	// Get the Influence function for OR components
	...
	// Get IF of the "treat" component
	inf_treat_pre 		= eta_treat_pre  :- (w10 :* att_treat_pre):/mean(w10) 
    inf_treat_post 		= eta_treat_post :- (w11 :* att_treat_post):/mean(w11)
	...
	// Now, the IF of control component
	real matrix inf_cont_pre, inf_cont_post

  
	inf_cont_pre 		= eta_cont_pre  :- w00 :* att_cont_pre :/ mean(w00)
    inf_cont_post 		= eta_cont_post :- w01 :* att_cont_post :/ mean(w01)
    
	real matrix M2_pre, M2_post, inf_cont_ps, M3_post, M3_pre, inf_cont_or_post, inf_cont_or_pre
	M2_pre 				= mean(w00 :* (y :- y0 :- att_cont_pre) :* xvar):/mean(w00)
    M2_post 			= mean(w01 :* (y :- y0 :- att_cont_post) :* xvar):/mean(w01)
    inf_cont_ps 		= lin_rep_ps * (M2_post :- M2_pre)'
	...
	// Now, we get the IF of the adjustment term
	real matrix inf_eff, mom_post, mom_pre, inf_or
  	inf_eff 			= ((eta_trt_post   :- w1  :* att_trt_post :/mean(w1))    :- 
						   (eta_trtt1_post :- w11 :* att_trtt1_post :/mean(w11))) :-
						  ((eta_trt_pre    :- w1  :* att_trt_pre:/mean(w1))     :- 
						   (eta_trtt0_pre :- w10 :* att_trtt0_pre:/mean(w10)))
	...
	real matrix inf_treat_or, inf_cont_or, inf_treat, inf_cont, trtr_eta_inf_func1, att_inf_func
...
//  xvar = st_data(.,xvar_, touse), J(rows(y),1,1)
    ...
    w10 = wgt :* trt :* (1 :- tmt)
    w11 = wgt :* trt :* tmt
    w00 = trimflag :* (wgt :* psc :* (1 :- trt) :* (1 :- tmt):/(1 :-  psc))   // Apply trimflag!
    w01 = trimflag :* (wgt :* psc :* (1 :- trt) :* tmt:/(1 :- psc)) :* trimflag  // Apply trimflag!
    w1  = wgt :* trt

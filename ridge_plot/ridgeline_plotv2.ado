THIS ONE TRIES TO PLOT ASIS. Not yet ready
...
program graphquery, rclass
	...
	local fany=stritrim(strtrim("`anything'"))
	tempname new
	capture frame create new
	qui:frame new: {
		import delimited using `"`r(fn)'"', clear
		keep   v1
		gen nwords= stritrim(strtrim(v1))
		local flag=0
		local cnt = 0
		local nn = _N
		while `flag'==0 & `cnt'<`nn' {
			local cnt=`cnt'+1
			local w1:word 1 of `=nwords[`cnt']'
			local w2:word 2 of `=nwords[`cnt']'
			local w3:word 3 of `=nwords[`cnt']'
			local w12="`w1'"+" "+"`w2'"
			if "`fany'"==`"`w12'"' {
				local flag =1
				local toreturn `w3'
...
program plotasis
	...
	NORMalize sum /// asis. use data as given; sum to get sums of data; norm
	addplot(string asis) * ]     //  overall look of outline
	...
	*************************
		...
		**Rename variables so I can work with them
		...
		if "`stack'`stack100'`stream'`stream1'`gap0'"=="" {
		
			if "`normalize'"=="" {
				sum yvar_, meanonly
				replace yvar_=yvar_/r(max)
			}
			else {
				foreach i of local lvl {
					display in w "`i'"
					local cn=`cn'+1
					sum yvar_ if over_==`i', meanonly
					replace yvar_=yvar_/r(max) if over_==`i'
				}
			}
			...
			********************************************************************
			...
			foreach i of local lvl {
				local cn=`cn'+1
 				replace yvar_=yvar_/`cnt' * `dadj' +  1/`cnt'*(`cnt'-`cn') if over_==`i'
				replace g0   = 1/`cnt'*(`cnt'-`cn')
		...
		if "`stack'`stack100'"!="" {
			gsort xvar_ -over_
			by xvar_:gen tyvar=sum(yvar_)
			replace yvar_ = tyvar
			if "`stack100'"!="" {
				by xvar_:replace yvar_=yvar_/yvar_[_N]*100
			}
		}
		
		if "`stream'`stream1'"!="" {
			by xvar_:replace yvar_=yvar_-0.5*yvar_[_N]
		}
		
		local cn
		foreach i of local lvl {
			local cn=`cn'+1
 			local toline `toline' (line yvar_ xvar_ if over_==`i', sort)
 		}
		two `toline'
	...
	qui:frame `frame': {
		...
		** First get densities and find the MAX
		foreach i of local lvl {
			...
			sum yvar_ if over_==`i', meanonly
			replace `f`cn''=r(min) if `f`cn''<r(min) 

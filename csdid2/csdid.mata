...
void csdid::csdid_setup(){
	...
	if (length(ivar)==0) {
		...
		oid  = 1::rows(yvar)	
		ivar = oid 
        // Ivar here does not matter, its just a sorting variable
		...
		type_data = 1
		oid  = 1::rows(yvar)
        if (length(cvar)>0) ord = order((cvar,ivar,tvar),(1,2,3))
		else                ord = order((ivar,tvar),(1,2))
		
		if (ord!=oid) {
            // Order all variables in case order changed
			yvar=yvar[ord,]
			tvar=tvar[ord,]
			gvar=gvar[ord,]
            ivar=ivar[ord,]
            // This runs only if they exist
            if (length(xvar)>0) xvar=xvar[ord,]
			if (length(wvar)>0) wvar=wvar[ord,]
 			if (length(cvar)>0) cvar=cvar[ord,]

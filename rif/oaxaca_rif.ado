...
program oaxaca_rif, eclass sortpreserve byable(recall)  properties( svyb )
   ...
   syntax anything [if] [in] [aw fw iw pw] , by(varname) rif(string)  ///
						[old swp swap Weights(str) rwlogit(str) rwipt(str) rwprobit(str) wgt(int -1) cluster(varname) robust ///
   ...
   *iseed undocumented. The idea is to make some indices reproducible
	...
	****
	if (("`rwlogit'"!="") + ("`rwprobit'"!="") + ("`rwipt'"!=""))>1  {
	display "Only one probability model can be set. Choose either Logit, probit or ipt for the first stage"
	exit
	}
    ****

...
program define _grifvar, sortpreserve
	...
	* Syntax sextion. Indicates what can be used and what cannot
	...
	pov(str) rich(str)  pline(str) rline(str) watts(str) sen(str) tip(str) 		/// This will contain the options from Essama-NssahLambert  
	...
	** This checks how many options have been selected	Only 1 option is allowed
	local d=("`gini'"!="")+("`ginismall'"!="")+("`mean'"!="")+("`var'"!="")+("`std'"!="")+("`cvar'"!="")+("`q'"!="")+("`q2'"!="")+("`iqr'"!="")+("`iqr2'"!="")+("`iqratio'"!="")+ ///
	        ("`glor'"!="")+("`lor'"!="")+("`ucs'"!="")+("`iqsr'"!="")+("`mcs'"!="")+("`pov'"!="")+("`rich'"!="")+("`entropy'"!="")+("`atkin'"!="")+ /// 
	...
	** This section will Test if the options are correctly specified. Right now only checks for poverty
	...
	if "`rich'"!=""  {
	   if "`rline'"=="" {
	   display "Requires to specify a rich line (value or variable)"
	   exit
	   }
	}
	...
	**RIF RICH. Flexible to admit any value for pov (0 headcount 1 poverty gap 2 poverty severity)
	** can also use either a fixed poverty line or a variable poverty line
    if "`rich'"!="" & "`rline'"!="" {
       numlist "`rich'", min(1) max(1)  range(>=0 )
		replace `touse'=. if `rline'==.
	   ** pov provides degree of sensitivity. pline the poverty line	   
 	   gen `typlist' `varlist'=.
	   * This estimates the standardized/censored income distribution
	   tempvar incz
	   gen double `incz'=(`rline'<`exp')*(`exp'-`rline')/`rline' if `touse'
	   if `rich'==0 {
	    qui:replace `varlist'=(`rline'<`exp') if   `touse'
	   }
	   else {
	    qui:replace `varlist'=`incz'^`rich'
	   }
	   label var `varlist' "RIF for FGT rich with alpha=`pov' and pline:`pline' of `exp'"
	}	

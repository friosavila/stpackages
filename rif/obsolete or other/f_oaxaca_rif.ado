...
program f_oaxaca_rif, eclass sortpreserve byable(recall) 
    if replay() {
        display_ob
        exit
    }
   ...
   syntax anything [if] [in] [aw fw iw pw] , by(varname) rif(string)   [ swp swap Weights(str) rwlogit(str) rwprobit(str) wgt(int -1) cluster(varname) ///
								relax Noisily scale(real 1.0) retain(str) replace iseed(str) s2var(varlist) initb(str) ]
...
*/	
	...
	*** Option 1. Standard oaxaca
	...
	preserve
		...
		*qui:reg `re' if `touse'==1
		qui:oaxaca `rif_var' `rest' `re' [`weight'=`exp'] if `touse'==1,   by(`by') w(`wgt')   `relax'  nose
 		drop `rif_var'
		local lgd "" `e(legend)'  ""
		tempname b V
		matrix `b'=e(b)
	...
	************************************************************
	preserve
		...
		** Here we do the probit/logit regression
		if "`rwprobit'"!="" {
			qui: `noisily' probit `dy' `rwprobit' [pw=`exp'] if `touse'==1, from(`initb')
			tempvar pr
		...
		if "`rwlogit'"!="" {
			qui: `noisily' logit `dy' `rwlogit' [pw=`exp'] if `touse'==1,  from(`initb')
			tempvar pr
			qui:predict double `pr', pr
			tempname b_rw v_rw
			matrix `b_rw'=e(b)
		...
		** re is the created s2var
		qui:oaxaca `rifvar' `rest'  `re' [aw=`exp'*`ipw'] if `touse'==1 & (`ddy'==1 | `ddy'==3),   by(`ddy') w(`wgt') nodetail /*robust cluster(`idcluster')*/ nose `relax'  
		tempname b0 v0 bb vb bx vx bc vc
        matrix `b0'=e(b)
        
		if `wgt'==0 { 
		   ** Delta B
	 		qui:oaxaca `rifvar' `rest'  `re' [aw=`exp'*`ipw'] if `touse'==1 & inlist(`ddy',1,2),   by(`ddy') w(0) /*cluster(`idcluster')*/ nose  `relax'  
			matrix `bb'=e(b)
			
		   ** Delta x
			qui: oaxaca `rifvar' `rest'  `re' [aw=`exp'*`ipw'] if `touse'==1 & inlist(`ddy',2,3),   by(`ddy') w(0) /*cluster(`idcluster')*/ nose  `relax' 
			matrix `bx'=e(b)
		...
		if `wgt'==1 {
		   ** Delta X
			qui:oaxaca `rifvar' `rest'  `re' [aw=`exp'*`ipw'] if `touse'==1 & inlist(`ddy',1,2),   by(`ddy') w(1) /*cluster(`idcluster')*/ nose `relax'  
			matrix `bx'=e(b)
			
		   ** Delta B
			qui:oaxaca `rifvar' `rest'  `re' [aw=`exp'*`ipw'] if `touse'==1 & inlist(`ddy',2,3),   by(`ddy') w(1) /*cluster(`idcluster')*/ nose `relax'  
			matrix `bb'=e(b)
    ...
    *display "is it not doing this"
	ereturn post `b'  , esample(`touse') depname(`y')
	eret loc title "Blinder-Oaxaca RIF-decomposition"
	eret loc model "Blinder-Oaxaca RIF-decomposition"
    eret loc cmd    "f_oaxaca_rif"
	eret loc cmdline  "f_oaxaca_rif `0'"
    eret loc depvar  "`y'"
	eret loc by  "`by'"

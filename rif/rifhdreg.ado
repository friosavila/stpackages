*! rifhdreg 2.6 Sep 2022 Addint Version and level
* rifhdreg 2.57 Oct 2021 by baseoutcome
* rifhdreg 2.56 Oct 2021 by FRA IPT!
...
program rifhdreg, eclass sortpreserve byable(recall) properties( svyb )  
	...
	[old retain(str)  replace  /// This is used to save the RIF, and replace if the variable already exists
	 ...
	 over(varlist)   /// Indicates to estimate the RIF over two groups. This is can be considered as a partially conditional model. (it is conditional on one variable only)
	 rwlogit(str)   rwprobit(str) rwipt(str) /// This two options are used to estimate the IPW. 
	...
	if "`rif'"=="" {
		display in red "option rif() required"
		error 198
	 }
...
qui {
	...
	** We first need to see if the probit model needs to be estimated
	if (("`rwlogit'"!="")+("`rwprobit'"!="")+("`rwmlogit'"!="")+("`rwmprobit'"!="")+("`rwipt'"!=""))>1 {
	...
	*** if Over has 2 values then:
	*if "`rwlogit'"=="" {	}
	
	if "`rwlogit'"!="" | "`rwprobit'"!="" | "`rwipt'"!="" {
	   if "`over'"=="" {
	   ...
	   qui:sum `dy' if `touse'
		...
		if "`rwipt'"!="" {
			version 14
			qui:`isily'  mlexp (`dy'*{xb:`rwipt' _cons}-(`dy'==0)*exp({xb:}))	///
				     if `touse'  [iw = `exp2'], vce(robust) 
			tempvar pr
			qui:predict double `pr'
			qui:replace `pr'=logistic(`pr')
			tempname b_rw v_rw
			matrix `b_rw'=e(b)
			matrix `v_rw'=e(V)
			local rwmodel="ipt"
	...
	* THis will do the multivalued treatment effect
	if "`rwmlogit'"!="" | "`rwmprobit'"!="" {
	  ...
	  tempvar dy
		...
		local maxv=r(max)
		if "`rwmprobit'"!="" {
			sum `dy' if `touse', meanonly
			qui:mprobit `dy' `rwmprobit' [pw=`exp2'] if `touse'==1, baseoutcome(`=r(min)')
			forvalues i =1/`maxv' {
				tempvar pr`i'
		...
		if "`rwmlogit'"!="" {
			sum `dy' if `touse', meanonly
			qui:mlogit `dy' `rwmprobit' [pw=`exp2'] if `touse'==1, baseoutcome(`=r(min)')
 			forvalues i =1/`maxv' {
				tempvar pr`i'
				qui:predict double `pr`i'', pr eq(`i')
			}

...
void mmqreg_vce1x(string scalar yvar_,  string scalar xvar_,	
		...
		// and Observations N obs k rows
		nobs = rows(xvar)
		k    = cols(xvar)
		// This gets all Q's fden qth qval
		...
		mata:if33b = -if33a :- (if1)*mean(x:/xg)' :- (q1:* if2b)*mean(x:/xg)'

 		for(i=1;i<=qs;i++) {
			
			w[.,i]=  1/fden[i]*(qth[i]:-((u:-qval[i]):<=0)) :- u:/mean(u_hat) :- (q*v):/mean(u_hat) 

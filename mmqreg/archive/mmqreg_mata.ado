*! v1.6 Small improvements estimation efficiencies
...
capture program drop mmqreg_mata
program define mmqreg_mata, eclass 
 ...
 if replay() {
	...
	syntax varlist(fv) [in] [if]  ,  /// Standard syntax
...
program define mmqreg1, eclass sortpreserve
	...
	** this code does all the rest. 
	...
	mata:mmqreg_vce1`x'("`y'","`xvar' `cns'","`lfb'","`sfb'", ///
		...
		sort `cluster'
		noisily:mata:mmqreg_vce1c("`y'","`xvar' `cns'","`lfb'","`sfb'", ///
...
void mmqreg_vce1x(string scalar yvar_,  string scalar xvar_,	
		...
		real matrix xvar_u_hat, omg_x
		xvar_u_hat = xvar:*u_hat
		omg_x=(xvar_u_hat:*u,xvar_u_hat:*v,u_hat:*w)
...
void mmqreg_vce1c(string scalar yvar_, string scalar xvar_,	
	   ...
	   iqxx=invsym(qxx)
		...
		real scalar nc, dfr,one
		  ...
		  nc   = rows(info)
		  dfr  = nc - 1
		  real matrix u_hat_i, u_u , u_v, u_w, v_v, v_u, w_w
		  
        for(i=1; i<=nc; i++) {
			xi      = panelsubmatrix(xvar ,i,info)
			u_hat_i = panelsubmatrix(u_hat,i,info)
			one   = J(rows(xi),1,1)
			uhatu = quadcross(xi ,u_hat_i:*panelsubmatrix(u,i,info))'
			uhatv = quadcross(xi ,u_hat_i:*panelsubmatrix(v,i,info))'
			uhatw = quadcross(one,u_hat_i:*panelsubmatrix(w,i,info))

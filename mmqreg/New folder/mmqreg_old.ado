...
void mmqreg_vce1x(string scalar yvar_, string scalar xvar_,	
	   ...
	   iqxx=invsym(qxx)
		...
		//omgx=(xvar:*u_hat:*u,xvar:*u_hat:*v,u_hat:*w)
...
real matrix xvar, yvar
	   ...
	   iqxx=invsym(qxx)
        ...
        for(i=1; i<=nc; i++) {
			...
			one   = J(rows(xi),1,1)
			uhatu = quadcross(xi ,panelsubmatrix(u_hat,i,info):*panelsubmatrix(u,i,info))'
			uhatv = quadcross(xi ,panelsubmatrix(u_hat,i,info):*panelsubmatrix(v,i,info))'
			uhatw = quadcross(one,panelsubmatrix(u_hat,i,info):*panelsubmatrix(w,i,info))

            //uhatu = panelsubmatrix(u_hat:*u,i,info)
			//uhatv = panelsubmatrix(u_hat:*v,i,info)
			//uhatw = panelsubmatrix(u_hat:*w,i,info)
			
			omgs= ( quadcross(uhatu,uhatu) , quadcross(uhatu,uhatv) , quadcross(uhatu,uhatw) \ ///
			        quadcross(uhatv,uhatu) , quadcross(uhatv,uhatv) , quadcross(uhatv,uhatw) \ ///
			        quadcross(uhatw,uhatu) , quadcross(uhatw,uhatv) , quadcross(uhatw,uhatw) )

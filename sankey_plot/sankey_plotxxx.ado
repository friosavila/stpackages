...
program sankey_wide
	syntax varlist (min=2) [if],  [width(varname) tight newframe(name) drop * label0(varname) ]
	if "`newframe'"=="" tempname newframe
	capture frame `drop' `newframe'
	frame put `varlist' `width' `label0' `if', into(`newframe')
	  ...
	  frame `newframe':{
		...
		foreach i in `varlist' {
			...
			if _rc!=0 {
				...
				capture decode `i', gen(xx`cnt')
				if _rc!=0 {
					qui:tostring `i', gen(xx`cnt')
				}
				local vlist  `vlist' xx`cnt'
			}
		}
		...
		*local vlist var1 var2 var3 
		...
		if "`tight'"!="" {
			...
			replace `width'=`wt'
			keep _x0 _x1 _y0 _y1 `width' `varlist' `label0'
			duplicates drop
		}
		keep _x0 _x1 _y0 _y1 `width' `varlist' `label0'
		list
		display "sankey_i2 	_x0 _y0 _x1  _y1 , width0(`width') `options' extra adjust label0(`label0') "
		sankey_i2 	_x0 _y0 _x1  _y1 , width0(`width') `options' extra adjust label0(`label0')  
	}
end 
